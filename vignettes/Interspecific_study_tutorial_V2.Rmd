...
mmo_pca <- PCAplot(mmo_subset, normalization = 'None', save_output = FALSE)
...
beta_diversity_dreams_bray <- GetBetaDiversity(mmo, method = 'bray', 
 normalization = 'None', distance = 'dreams',filter_id=F,filter_group=F)
...
Option 3... use each embedding bit of the dreams embeddings vector as a single variable... 
run cladistics...
build a phyl from quantatative traits
#Not yet, but todo is to figure out what models can take these types of data...

#Option 4:

Use Molecular Communities - this will produce a refined molecular network... in graphml format. 

nodes + edges. We then have to convert this into a distance matrix or tree...



Options 4B
  connect the islands based on centroid (or representative compound near centroid, subset of most connected comps) to generate a communitie x community pairwise backbone for the dendrogram.
  
  ### ask if this already exists? We have islands. We want to connect them by centroids.
  


#### To Do Items:
1) Process Panama Data w/ updated metadata - Done
2) Generate a molecular network - hairball or not... [Dale]
3) Run Molecular communities - to generate graphml...[Dale]

4) Visualize graphml [Guillaume will explore]
    -color code communities 
    -color by npclassifier labels
    
5) Convert Graphml to distance matrix - hopefully easy [Guillaume]
  -compare compound pairwise distance methods
  -should explore converting to tree... but not necesary.
6) Use distance matrix in ecomet/for cscs

7) Decide to pursue centroid of communities or not...
